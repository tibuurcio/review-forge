...
export function useDiffComments() {
  ...
  const addComment = useCallback((changeKey: string, content: string = '') => {
  ...
  const saveEdit = useCallback((commentId: string, content: string) => {
    const mayUpdate = (comment: CommentData): CommentData => {
      if (comment.id !== commentId) {
        return comment
      }
  ...
  const deleteComment = useCallback((commentId: string) => setComments(s => s.filter(v => v.id !== commentId)), [])
